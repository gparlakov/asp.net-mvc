    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication
                    .CheckPasswordAndSignInAsync(AuthenticationManager, model.UserName, model.Password, model.RememberMe);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Success)
                {
                    var role = await IdentityManager.Roles.FindRoleByNameAsync("user");
                    if (role == null)
                    {
                        var createUserRoleResult = await IdentityManager.Roles
                            .CreateRoleAsync(new Role { Name = "user" });

                        if (createUserRoleResult.Success)
                        {
                            role = await IdentityManager.Roles.FindRoleByNameAsync("user");
                        }
                        else
                        {
                            AddErrors(createUserRoleResult);
                        }
                    }

                    await IdentityManager.Roles.AddUserToRoleAsync(user.Id, role.Id);
                    await IdentityManager.Authentication
                        .SignInAsync(AuthenticationManager, user.Id, isPersistent: false);
        ...
        {
            string message = null;
            IdentityResult result = await IdentityManager.Logins
                .RemoveLoginAsync(User.Identity.GetUserId(), loginProvider, providerKey);
        ...
        public async Task<ActionResult> Manage(string message)
        {
            ...
            ViewBag.HasLocalPassword = await IdentityManager.Logins
                .HasLocalLoginAsync(User.Identity.GetUserId());
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication.CreateAndSignInExternalUserAsync(AuthenticationManager, new AppUser{UserName =model.UserName});
                if (result.Success)
                {
                    return RedirectToLocal(returnUrl);
                }
